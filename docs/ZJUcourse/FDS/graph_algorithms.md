...
DFS 是前序遍历的一种推广，基本模板如下：

```c
void DFS ( Vertex V )
{
    visited[ V ] = true;    // 标记该顶点已访问，以避免死循环 
    for ( each W adjacent to V ) // 对于每一个与 V 相邻的顶点 W
        if ( !visited[ W ] )     // 如果 W 还没有被访问过
            DFS( W );            // 递归调用 DFS 访问 W
}
```

如果图是用邻接表存储的话，那么以上 DFS 的时间复杂度为
...
??? code "代码实现"
    ...
    {
        for ( each V in G )       // 对于图中的每一个顶点 V
            if ( !visited[ V ] ) { // 如果 V 还没被访问过
                DFS( V );          // 从 V 开始做深度优先搜索
                printf("\n");      // 打印换行符（说明一个连通分量找完了）
            }
    }
...
- 关节点 Articulation Point ：从图中删除该点后，得到的图至少有 2 个连通分量。

- 双连通图 Biconnected Graph ：一个图本身连通，并且没有任何关节点。

- 双连通分量 Biconnected Component ：一个极大的双连通子图，再多任何一条边或任何一点就不满足双连通性质了。

!!! note "性质"

    图中的任何一条边，一定属于且仅属于一个双连通分量。

#### 寻找无向图中的双连通分量

1. 使用 DFS 先获得一颗图 G 的生成树

    ![dfs生成树](dfs_spanningt.png)
    
    生成树的每个点都有一个蓝色数字 `Num(v)` ，代表点 $v$ 在 DFS 中被访问的顺序。

    ![dfs生成树](dfs_spanningt2.png)

    在生成树中，用虚线连接原图中没有被覆盖的边，称为回边 back edge 。

    !!! note "DFS 生成树的性质"

        - 如果点 $u$ 是点 $v$ 的祖先，那么 `Num(u)` < `Num(v)` 。

        - 回边连接的必然是某个点与其祖先。

2. 在生成树中找到图 G 的关节点

    - 根节点如果有超过 1 个孩子节点，那么就是关节点；

    - 非根节点如果有孩子节点，并且子树中的任何节点都没有回边连向它的祖先，那么就是关节点。

    !!! note "如何用代码实现"

        我们定义一个变量 `Low(u)` ，记录点 $u$ 及它的子树能够连接到的所有祖先节点中 `Num` 最小的节点的 `Num` 值，例如刚刚的生成树记为
        
        ![生成树low值](low.png)

        现在，判断点 $u$ 为关节点的规则是：

         - $u$ 为根节点，并且有超过 1 个孩子节点；

         - $u$ 为非根节点，并且有孩子节点 $child$ 满足 `Low(child)` $\ge$ `Num(u)` 。

### 欧拉回路 Euler Circuits

#### 定义

- 欧拉路径 Eulerian Path ：不离开纸面，经过每条边一次。

- 欧拉回路 Eulerian Circuit ：不离开纸面，经过每条边一次，并且回到出发点。

!!! note "判断方法"

    - 如果图连通，并且每个点的度数都是偶数，那么存在欧拉回路。

    - 如果图连通，并且恰好有 2 个点的度数为奇数，其他为偶数，那么存在欧拉路径，起点和终点就是这 2 个点。

#### DFS 实现欧拉回路

- 每次 DFS 得到一个回路，只要有点没有覆盖，就继续 DFS ，最后把得到的回路连接；

- 用链表维护路径，方便连接；

- 对于每个点的邻接表，维护一个指针，指向其中最后一个被访问过的点。
