...
## 9 桶排序 Bucket Sort 与基数排序 Radix Sort

### 桶排序

**问题**：假设我们有 $N$ 个学生，每个学生都有一个范围在 0 到 100 之间的分数（共有 $M = 101$ 种不同可能的分数），如何在线性时间内根据他们的成绩进行排序？

桶排序的解决办法：
```c
{
      initialize count[ ]; 
      // 初始化一个名为 count 的数组（也就是一排空桶）
    
      while ( read in a student record )
            insert to list count[stdnt.grade];
      // 当读入一个学生的数据时，直接把这个学生塞进下标为 stdnt.grade（学生成绩）的那个桶里
    
      for (i=0; i<M; i++) {
            if (count[i])
                  output list count[i];
      }
    // 从第 0 个桶一路扫描到第 M-1 个桶，如果桶里有学生，就把桶里的学生挨个输出出来
}
```
这样时间复杂度为 $T(N, M) = O(M + N)$ 。

!!! warning "桶排序的问题"

      如果 $M >> N$ ，即桶的数量远大于待排序元素的数量，那么在空间上非常浪费。

### 基数排序

>弥补桶排序的问题。

**问题**：给定 $N = 10$ 个范围在 0 到 999 之间的整数（即 $M = 1000$），能否在线性时间内对它们进行排序？

#### LSD 策略

从最低有效位开始排序 Least Significant Digit first ，记 $B$ 为桶的数量，$P$ 为分配的轮数，每一轮只按一个位数放到对应桶中。

!!! example "例子"

      对输入 `64, 8, 216, 512, 27, 729, 0, 1, 343, 125` 进行排序。

      由于位数最多的数字有三位，因此这里取 $P = 3$ ；又因为是十进制数，取 $B = 10$ ，从个位开始分配：
      ![LSD 的一个例子](LSD.png)

这样，时间复杂度为 $T = O(P(N + B))$ 。

#### 字典序 Lexically Sorted

基数排序不仅可以应用到纯数字上，也可以拓展到多键值排序上，意思是只要一个键值有明确优先级，就可以排序。

假设每个数据 $R_i$ 有 $r$ 个不同的键值，记 $K_i^j$ 为第 $i$ 个数据的第 $j$ 个键值（$j = 0$ 时优先级最高）。 

- Most Significant Digit Sort 从 $K^0$ 开始分配；

- Least Significant Digit Sort 从 $K^{r-1}$ 开始分配。 